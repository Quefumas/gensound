...
class Transform:
    ...
    def num_samples(self, sample_rate):
        if not hasattr(self, "duration"):
            raise TypeError(
                "transform.duration must be defined to support conversion to samples")
...
class Combine(Transform):
    ...
    def realise(self, audio):
        ...
        max_channel = max(0, self.channel_slice.start or 0,
                          (self.channel_slice.stop or 0)-1)
        ...
        audio.audio[self.channel_slice, start_sample:start_sample +
                    new_audio.length+new_audio.shift] += new_audio.audio[:, -new_audio.shift:]
...
class FadeIn(Transform):
    ...
    def realise(self, audio):
        ...
        if self.curve == "lin":
            ...
            amp = DB_to_Linear(np.linspace(
                FadeIn.min_fade, 0, self.num_samples(audio.sample_rate)))
...
class FadeOut(Transform):
    ...
    def realise(self, audio):
        ...
        if self.curve == "lin":
            ...
            amp = DB_to_Linear(np.linspace(
                FadeOut.min_fade, 0, self.num_samples(audio.sample_rate)))
        # amp = np.linspace(0, 1, self.num_samples(audio.sample_rate))
        # perhaps the fade in should be nonlinear
        # TODO subsciprability problem
    ...
    def realise(self, audio):
        ...
        for (i, dB) in enumerate(dBs):
            if isnumber(dB):
                ...
                audio.audio[i, dB.num_samples(
                    audio.sample_rate):] *= DB_to_Linear(dB.endpoint())
            else:
    ...
    def realise(self, audio):
        ...
        for (i, amp) in enumerate(amps):
            ...
            if isnumber(amp):
                ...
                audio.audio[i, amp.num_samples(
                    audio.sample_rate):] *= amp.endpoint()
            else:
                raise TypeError("Unsupported amplitude type")
...
class SineAM(Transform):
    ...
    def realise(self, audio):
        ...
        assert isinstance(
            audio, Audio)  # TODO remove this after debug hopefully
...
class Limiter(Transform):
    ...
    def __init__(self, max_amplitude=None, max_ratio=None, max_dB=None,
        ...
        assert (max_amplitude is not None) + \
            (max_ratio is not None) + (max_dB is not None) <= 1
        assert (min_amplitude is not None) + \
            (min_ratio is not None) + (min_dB is not None) <= 1
        ...
        self.is_min = not (
            min_amplitude is None and min_ratio is None and min_dB is None)
        self.is_max = not (
            max_amplitude is None and max_ratio is None and max_dB is None)
    ...
    def realise(self, audio):
        ...
        if self.is_max:
            ...
            np.clip(audio.audio, -self.max_amplitude,
                    self.max_amplitude, out=audio.audio)
        ...
        if self.is_min:
            ...
            audio.audio[:, :] = np.sign(audio.audio) * \
                np.clip(np.abs(audio.audio),
                        a_min=self.min_amplitude, a_max=None)
...
class Pan(Transform):
    ...
    def pan_shape(x): return np.log(x/Pan.width + 0.5) * \
        (-Pan.panLaw / np.log(2))  # +0.1 to prevent log(0)

    def LdB(x): return Pan.pan_shape(-x)
    def RdB(x): return Pan.pan_shape(x)
    def defaultStereo(x): return (Pan.LdB(x), Pan.RdB(x))
    ...
    def realise(self, audio):
        ...
        for (i, dB) in enumerate(dBs):
            if isnumber(dB):
                ...
                audio.audio[i, len(
                    dB):] *= DB_to_Linear(self.scheme(self.pan.endpoint())[i])
...
class Stretch(Transform):
    ...
    def realise(self, audio):
        assert (self.rate is None) + \
            (self.duration is None) == 1, "Stretch: exactly one of the arguments rate, duration, must be defined."
        ...
        audio.audio = interpolate(
            audio.audio, np.arange(0, audio.length-1, factor))
...
class Downsample(Transform):
    ...
    def realise(self, audio):
        ...
        for i in range(1, self.factor):
            ...
            audio.audio[:, i::self.factor] = audio.audio[:,
                                                         0:l + (-self.factor if less else 0):self.factor]
...
class Convolution(Transform):
    ...
    def __init__(self, response):
        # Audio, direct buffer, or filename
        assert isinstance(response, (Audio, np.ndarray, str))
    ...
    def realise(self, audio):
        ...
        if self.response.shape[0] == 1:  # apply same mono reverb to all channels
            for i in range(audio.num_channels):
                audio.audio[i, :] = convolve(
                    audio.audio[i, :], self.response[0, :], mode="same")
            ...
            for i in range(self.response.shape[0]):
                audio.audio[i, :] = convolve(
                    audio.audio[i, :], self.response[i, :], mode="same")
